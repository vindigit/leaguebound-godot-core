"""Read-only Validation A construction audit; no outcome or Validation B analysis."""
...
for c in audit['competitions']:
 ...
 for version in ('baseline','candidate'):
  ...
  for arm in ('home','neutral'):
   label=f'validation_a_{version}_{arm}_{comp}'
   ...
   for offset,row in enumerate(data['rows']):
    v=28080000+offset;assert row['variation']==v and row['seed']==v+1 and row['block']==v//4
...
print(json.dumps({'verdict':'VALIDATION A FIXTURE CONSTRUCTION ACCEPT','cells':rowsout,'source_drift_since_gate':False,'scope':'No outcome fields analyzed; no Validation B read'},indent=2))
