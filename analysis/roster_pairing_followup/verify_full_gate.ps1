...
$start = 'bf275ccc89e16fe75830fd34a37529faf8df610a'
$logPath = Join-Path $PSScriptRoot 'full_gate.txt'
$xmlPath = Join-Path $PSScriptRoot 'full_gate_results.xml'
$log = (Get-Content -LiteralPath $logPath -Raw) -replace "`e\[[0-9;]*m", ''
$result = Get-Content (Join-Path $PSScriptRoot 'full_gate_result.json') -Raw | ConvertFrom-Json
...
if ($total -ne [int]$xml.testsuites.tests -or $total -ne 692 -or $rows.Count -ne 55) { throw 'Unexpected full suite totals' }
...
foreach ($required in @('Parse check: 266 script(s) checked, 0 failure(s)','LeagueBound headless acceptance: PASS','Invariants: PASS','Builder calibration: PASS','Builds evaluated: 810','80 metric(s), 80 judged, 0 failures: PASS','15 metric(s), 15 judged, 0 failures: PASS','Overall Summary: 692 test cases | 0 errors | 0 failures | 0 flaky | 0 skipped | 0 orphans |','Executed test suites: (55/55)','Executed test cases : (692/692)','Exit code: 0','Run tests ends with 0','=== All requested checks passed')) { if (-not $log.Contains($required)) { throw "Missing completion proof: $required" } }
