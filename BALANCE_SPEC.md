...
| §14.1 competition bands | **Ten of eleven pass at top domestic; all five levels re-measured on one untouched range** | Every level was re-measured at 400 games on variations 60,000-60,399, first under the `simulation-v3-margin` ruleset and again under `simulation-v4-management`, so no figure in this table predates its own engine any more. Points per possession moved from 1.2084 to **1.1680** — inside 1.08-1.18 on the judged range, and 1.1771 ±0.0040 pooled over 2,100 population games, which is inside the band with its interval touching the ceiling. It was corrected at its measured source: field-goal attempts per possession ran at 0.967 against a band-consistent 0.90, and the two processes that set that number are offensive rebounding and ball-handling disruption. Points per possession is 1.1691 under `simulation-v4-management`, eleven ten-thousandths from where §5.10 left it, because the score-and-clock correction changes which action a coach selects and not how efficient anybody is. Assist percentage is the only remaining §14.1 failure at top domestic — 0.4854 against 0.52-0.72, statistically unchanged and belonging to the assist model. The other four levels: high school 10/10, overseas 9/10, development 9/10, college 8/10, with college field-goal percentage at 0.4151 against a 0.42 floor and assist percentage failing at all four. `PROJECT_STATUS.md` §5.10. |
| §14.2 game-shape targets | **Close-game share now passes at all five competition levels; blowout and overtime pass at high school.** Blowout 0.1400 ✓ / 0.1750 ✓ / 0.2550 / 0.1950 / 0.2450 and overtime 0.0450 ✓ / 0.0250 / 0.0275 / 0.0375 / 0.0225 across high school, college, development, overseas and top domestic at 400 games each; close-game share 0.2825 / 0.2700 / 0.2200 / 0.3000 / 0.2225, all inside 0.22–0.34. Two untouched 500-game population validation ranges give blowout 0.2660 and 0.2840, close 0.1880 and 0.2040, overtime 0.0260 and 0.0200. `PROJECT_STATUS.md` §5.10 and §5.11. | **§5.10's premise was measured and turned out to be true of the engine and false of basketball.** `Var(A − B) = Var(A) + Var(B) − 2·Cov(A, B)` closes exactly on the measured columns, and the covariance was +4.67 on a zero-gap mirror fixture — a correlation of 0.026. The margin's own path confirmed it: the variance ratio, corrected against a permutation control that carries the finite-sample artefact and none of the behaviour, measured 1.00 at every horizon from one possession to eighty. The margin was a random walk because **no engine mechanism created any covariance at all**: a team fifteen points ahead consumed *less* clock than the team chasing it, hunted the same share of threes, and outscored it. Three mechanisms the specification already authorised were structurally absent — §10.2/§10.3/§18.2 score-and-clock game management, §20.2 end-of-regulation possession strategy, and §4/§5 coaching timeouts. Implementing them raised the mirror-fixture covariance to +25.12 and cut the margin's variance from 347.3 to 288.8, seventy per cent of it through covariance, with no probability touched anywhere: the same shot from the same shooter resolves identically at +30 and −30. **What remains is quantified rather than argued.** A ledger-faithful replay model, rebuilt from each game's own possession-outcome distribution and validated against the engine it models, puts the blowout ceiling at a leading-team penalty of about 0.12 points per possession — roughly a tenth of scoring efficiency, which is the real magnitude of garbage time — against a measured 0.008–0.020. The authorised mechanisms saturate below it, and pushing every one of them to the top of its safe range was tried on the tuning range and bought nothing. **The overtime band is unreachable by any dispersion mechanism whatever:** the model's tie rate never reaches 0.04 even at a margin standard deviation of 9.9, because the probability of an exactly level score is the margin's density at zero. The remaining gap is an owner question with three costed options, and no band was rewritten to match the measurement. |
