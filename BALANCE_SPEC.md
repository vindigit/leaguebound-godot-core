...
**Implementation status: all five §8.4 bands now measure inside their locked targets, and doing so required the rare-generational cohort to run above these guardrails.** Summed across a twenty-three-season career, the guardrails above permit about 2,695 AP-equivalent; a rare-generational career reaching the §8.4 median of 92–95 was measured to need about 3,140, so that cohort runs roughly 16.5% above them, passing its guardrail in 17 seasons of 23. §8.4 is Locked and these bands are Baselines, so the tension resolves in §8.4's favour and is recorded rather than hidden: every season above a guardrail now triggers the balance warning this section requires and carries a source-ledger entry naming what produced the excess, and the career-progression report judges that every one of them is explained. Whether the correct long-run answer is higher professional bands, a differently phased rare-generational career, or accepting that this cohort's seasons are genuinely exceptional is an owner decision the measurement does not make. `PROJECT_STATUS.md` §5.8 carries it.
...
| §14.1 competition bands | Substantially converged, not certified; top domestic re-measured after the free-throw correction | From roughly fifty failing metrics to about ten. The top-domestic profile was re-measured at 400 games at commit `c57f34e`, because every earlier figure predated `00567d4` and tuning against them would have been tuning against a superseded engine. Nine of eleven §14.1 metrics pass. Assist percentage is **0.4815 ±0.0052** against 0.52–0.72 — statistically unchanged from the 0.482 recorded at 80 games, so the failure belongs to the assist model rather than to the pre-correction engine, and the creation-versus-attribution diagnosis can proceed against the current engine. Points per possession is **1.2084** against 1.08–1.18, five times the previous sample and further outside the band, so the earlier 1.186 understated the miss. The other four competitions have not been re-measured and their recorded figures remain stale. |
| §14.2 game-shape targets | **Assessable after all, and four of five fail** | Re-measured at 400 top-domestic games after the free-throw correction. The earlier "not assessable" entry was written when only tens of games had been run; at 400 the intervals are ±0.04 and the verdicts are real. Blowout share 0.3450 against 0.08–0.18, close-game share 0.1925 against 0.22–0.34, overtime rate 0.0175 against 0.04–0.08. These fail as one coherent pattern rather than independently — the game-to-game score margin is over-dispersed — and that is very likely the same defect that puts points per possession above its band. Home win rate 0.5625 is nominally outside 0.53–0.56 but its interval spans the band and it should not be treated as a separate failure at this sample. |
