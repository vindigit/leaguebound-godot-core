...
| §14.1 competition bands | **Ten of eleven pass at top domestic; all five levels re-measured on one untouched range** | Every level was re-measured at 400 games on variations 60,000-60,399 under the `simulation-v3-margin` ruleset, so no figure in this table predates its own engine any more. Points per possession moved from 1.2084 to **1.1680** — inside 1.08-1.18 on the judged range, and 1.1771 ±0.0040 pooled over 2,100 population games, which is inside the band with its interval touching the ceiling. It was corrected at its measured source: field-goal attempts per possession ran at 0.967 against a band-consistent 0.90, and the two processes that set that number are offensive rebounding and ball-handling disruption. Assist percentage is the only remaining §14.1 failure at top domestic — 0.4831 ±0.0053 against 0.52-0.72, statistically unchanged and belonging to the assist model. The other four levels: high school 10/10, overseas 9/10, development 9/10, college 8/10, with college field-goal percentage at 0.4151 against a 0.42 floor and assist percentage failing at all four. `PROJECT_STATUS.md` §5.10. |
| §14.2 game-shape targets | **Diagnosed to its mechanism, corrected in part, and now bounded rather than open.** Blowout share 0.2880 and 0.3100 on two untouched 500-game validation ranges against 0.08–0.18, close-game share 0.2100 and 0.2060 against 0.22–0.34, overtime 0.0220 and 0.0260 against 0.04–0.08. Before the correction: 0.3640, 0.1730, 0.0190. `PROJECT_STATUS.md` §5.10. | **The over-dispersion was two separate things and neither was the one §5.5 guessed.** The calibration fixture was stacking two team-strength spreads, one of them applied to the away roster only, which left 66% of the sample in the large-mismatch band and gave the pregame gap 28.8% of the margin variance; that is corrected, the fixture now owns one symmetric spread, and the population is 22% near-even / 62% modest / 15% large. The rest is not a defect. The pooled within-team-game variance of one possession's points is 1.59, a random walk of 100 such possessions per side implies a margin standard deviation of 17.9, and two identical rosters measure 18.4 — an over-dispersion ratio of 1.03, which rules out persistent team multipliers, hot/cold states, transition snowballing, rebound compounding, RNG coupling and cross-game leakage in one measurement. **§14.1 and §14.2 are not jointly satisfiable under an independent-possession model.** Across the corners of every §14.1 top-domestic band the implied per-possession variance runs 1.38–1.60, so the margin standard deviation cannot fall below about 16.3 and the blowout share cannot fall below about 22% against a ceiling of 18%; the overtime rate is bounded above by the density of that margin at zero, about 2.4% against a 4–8% band. The measurement agrees: after the fixture correction the near-even band measures 0.2232–0.2920 blowouts at margin standard deviations of 16.1–19.4. Closing either gap needs a mechanism that makes a team's later possessions worse *because* it leads, which §14.2's own "team totals must emerge from possessions" and `SIMULATION_SPEC.md` §20.3's no-comeback-script rule forbid. This is an owner question — move §14.2, move §14.1's possession or three-point bands, or authorise a bounded reversion under §20.3 — and no band was rewritten to match the measurement. |
